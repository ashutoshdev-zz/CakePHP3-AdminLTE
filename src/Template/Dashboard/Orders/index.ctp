...
<!--                <td><?= $this->Number->format($order->delivery_status) ?></td>-->
                <td class="actions">
                    ...
                    <?php //$this->Html->link(__('Edit'), ['action' => 'edit', $order->id],array('class'=>'btn btn-success')) ?>
                    <?php //$this->Form->postLink(__('Delete'), ['action' => 'delete', $order->id], ['confirm' => __('Are you sure you want to delete # {0}?', $order->id),'class'=>'btn btn-danger']) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
